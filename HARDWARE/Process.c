...
void ELine0() {
    ...
    if (obj[5] || !obj[3] || !obj[4]) {              // 如果在F
        ...
        if (obj[5]) {
            if (!obj[3] || !obj[4]) delay_ms(500);        // 防止左侧抓手撞倒高木桩
            Run(0, 630, MVEL);                            // 去H
        } else {
            Run(0, 375, MVEL);                 // 去G抓
            while (MotorState != Stop);        // 阻塞等待 车子停稳
...
void Catch(char Line, u8 odd, u8 mid, u8 even) {
    ...
    switch (Line) {
        case 'E':
        case 'c': angle_wai = S2; break;
        case 'F':
        case 'C': angle_wai = S2 - S1; break;
        default:  angle_wai = 0; break;
    }

    if (odd)
        while (Stepper_GetStatus(1) || Stepper_GetStatus(3));
    if (even)
        while (Stepper_GetStatus(2) || Stepper_GetStatus(4));
    if (mid)
        while (Stepper_GetStatus(5));
    ...
    if (odd)
        while (Stepper_GetStatus(3));        // 等待步进3向下移动完毕
    if (even)
        while (Stepper_GetStatus(4));        // 等待步进4向下移动完毕
    if (mid)
        while (Stepper_GetStatus(5));        // 等待步进5向下移动完毕
