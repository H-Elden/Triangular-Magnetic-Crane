...
void Speed_DOWN(float dccel) {
    ...
    if (Run_flag && MVEL_flag) dccel -= 300;
    if (LTargetVelocity_f > 0) {
    ...
    if (TIM_GetITStatus(TIM6, TIM_IT_Update) == 1) {        // 当发生中断时状态寄存器(TIMx_SR)的bit0会被硬件置1
        ...
        if (Fudu_flag) {
            ...
            if (Run_flag) {
                ...
                Speed_UP(Accel);
                if (MotorState == VelCir) Speed_DOWN(Dccel);
            }
        }
        Run_Dis += -1.0 * LEncoder / 54000 * 2 * PI * Radius;        // Run_Dis 为当前行进的距离
        if (Sensor_open) Getdis();
        switch (MotorState) {
            /*----闭环速度----*/
            case Stop: {
                LTargetVelocity    = 0;
...
void TIM6_Init() {
    ...
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM6, ENABLE);        // 使能通用定时器6时钟
    ...
    TIM_TimeBaseInit(TIM6, &TIM_TimeBaseInitStrue);                      // 根据TIM_TimeBaseInitStrue的参数初始化定时器TIM6

    TIM_ITConfig(TIM6, TIM_IT_Update, ENABLE);        // 使能TIM6中断，中断模式为更新中断：TIM_IT_Update

    NVIC_InitStrue.NVIC_IRQChannel                   = TIM6_IRQn;        // 属于TIM6中断
    ...
    NVIC_InitStrue.NVIC_IRQChannelSubPriority =
        0;        // 响应优先级为1级，值越小优先级越高，0级优先级最高【【【TIM6的中断优先级应高于TIM5，否则会出现卡顿】】】
    NVIC_Init(&NVIC_InitStrue);        // 根据NVIC_InitStrue的参数初始化VIC寄存器，设置TIM6中断
...
int Velocity_Restrict(int PWM_P, int TargetVelocity) {
    if (PWM_P > +TargetVelocity * 2)
        PWM_P = +TargetVelocity * 2;
    else if (PWM_P < -TargetVelocity * 2)
        PWM_P = -TargetVelocity * 2;
    else
        PWM_P = PWM_P;
    return PWM_P;
